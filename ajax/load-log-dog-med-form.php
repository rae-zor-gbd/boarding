...
if (isset($_POST['status']) AND isset($_POST['id'])) {
  $status=$_POST['status'];
  $medID=$_POST['id'];
  $sql_med_info="SELECT dogReservationID, medName, strength, dosage, frequency, notes FROM dogs_medications WHERE dogMedID='$medID'";
  $result_med_info=$conn->query($sql_med_info);
  $row_med_info=$result_med_info->fetch_assoc();
  $reservationID=$row_med_info['dogReservationID'];
  ...
  $sql_reservation_info="SELECT checkIn, DATE_ADD(checkOut, INTERVAL 1 DAY) AS checkOut FROM dogs_reservations WHERE dogReservationID='$reservationID'";
  ...
  foreach ($dateRange as $logDate) {
    ...
    $sql_log_info="SELECT givenAM, givenNoon, givenPM, l.notes FROM dogs_medications_log l JOIN dogs_medications m USING (dogMedID) WHERE l.dogReservationID='$reservationID' AND l.dogMedID='$medID' AND logDate='$logDateV1'";
