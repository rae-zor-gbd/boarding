...
if (isset($_POST['startDate']) AND isset($_POST['endDate'])) {
  ...
  $sql_room_count="SELECT COUNT(roomID) AS availableRooms FROM rooms WHERE roomID NOT IN (SELECT roomID FROM dogs_reservations WHERE checkIn<='$endDate' AND checkOut>='$startDate')";
  ...
  $sql_weekend_count="SELECT COUNT(dogReservationID) AS weekendCheckOuts FROM dogs_reservations WHERE checkIn<='$endDate' AND checkOut>='$startDate' AND WEEKDAY(checkOut) IN (4, 5, 6)";
