...
if (isset($_POST['medName'])) {
  ...
  $sql_strengths="SELECT CONCAT(strength, IF(unit='%', '', ' '), unit) AS strength FROM medications m JOIN medications_strengths s USING (medID) WHERE medName='$medName' ORDER BY FIELD(s.unit, 'MCG', 'MG', 'G', '%', 'ML'), s.strength";
