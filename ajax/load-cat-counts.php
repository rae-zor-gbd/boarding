...
if (isset($_POST['startDate']) AND isset($_POST['endDate'])) {
  ...
  $sql_condo_count="SELECT COUNT(condoID) AS availableCondos FROM condos WHERE condoID NOT IN (SELECT condoID FROM cats_reservations WHERE checkIn<='$endDate' AND checkOut>='$startDate')";
  ...
  $sql_weekend_count="SELECT COUNT(catReservationID) AS weekendCheckOuts FROM cats_reservations WHERE checkIn<='$endDate' AND checkOut>='$startDate' AND WEEKDAY(checkOut) IN (4, 5, 6)";
